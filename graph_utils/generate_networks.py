...
from utils.utils import (
	...
	add_upload_download_delays,
	get_delta_mbst_overlay,
	get_star_overlay,
	get_ring_overlay,
	get_matcha_cycle_time,
)

# Model size in bit
MODEL_SIZE_DICT = {
	"driving_udacity": 20157248,
	"driving_gazebo": 20157248,
	"driving_carla": 20157248,
}
...
parser.add_argument("name", help="name of the network to use;")
parser.add_argument(
	"--experiment",
	type=str,
	help="name of the experiment that will be run on the network;"
	"possible are femnist, inaturalist, synthetic, shakespeare, sent140;"
...
parser.add_argument(
	"--centrality", help="centrality type; default: load;", default="load"
)
...
if __name__ == "__main__":
	if args.experiment is not None:
		...
		args.computation_time = (
			args.local_steps * COMPUTATION_TIME_DICT[args.experiment]
		)
	...
	for u, v, data in connectivity_graph.edges(data=True):
		weight = (
			args.computation_time
			+ data["latency"]
			+ args.model_size / data["availableBandwidth"]
		)
	...
	cycle_time, _, _ = evaluate_cycle_time(
		add_upload_download_delays(MST, args.computation_time, args.model_size)
	)
	...
	# delta-MBST
	delta_mbst, best_cycle_time, best_delta = get_delta_mbst_overlay(
		connectivity_graph.copy(), args.computation_time, args.model_size
	)

	delta_mbst = add_upload_download_delays(
		delta_mbst, args.computation_time, args.model_size
	)
	...
	cycle_time, _, _ = evaluate_cycle_time(
		add_upload_download_delays(star, args.computation_time, args.model_size)
	)
	...
	# Ring
	ring = get_ring_overlay(
		connectivity_graph.copy(), args.computation_time, args.model_size
	)

	cycle_time, _, _ = evaluate_cycle_time(
		add_upload_download_delays(ring, args.computation_time, args.model_size)
	)
	...
	# MATCHA
	cycle_time = get_matcha_cycle_time(
		underlay.copy(),

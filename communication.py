...
class Network(ABC):
    ...
    def write_logs(self):
        ...
        train_loss, train_rmse, train_mae = self.global_model.evaluate_iterator(self.train_iterator)
        end_time_train = time.time()
        print('\t - test set')
        test_loss, test_rmse, test_mae = self.global_model.evaluate_iterator(self.test_iterator)
...
class Peer2PeerNetwork(Network):
    def mix(self, write_results=True):
        ...
        """
        # update workers
        param_name = [name for name, _ in self.global_model.net.named_parameters()]
        silo_batch_loss = []
        ...
        if self.model == "CDL_FADNet" and self.round_idx == 0:
            model.net.copy_weight_main2supp()
        ...
        # mix models
        for param_idx, param in enumerate(self.global_model.net.parameters()):
            if "supp" in param_name[param_idx] and self.model == "CDL_FADNet":
                pass
